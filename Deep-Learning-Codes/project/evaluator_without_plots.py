...
def evaluate_randomly(encoder, decoder):
    ...
    translated=[]
    bs=0.0
    n_sentence=min(len(pairs),10000)
    for i in range(n_sentence):
        ...
        translated.append(output_words)
    bleu=nltk.translate.bleu_score.corpus_bleu(actual,translated)
        #actual.append([pair[1].split(' ')])
        #translated.append(output_words)
        #bs =bs+bleu
    #bleu, precisions, bp, ratio, translation_length, reference_length= compute_bleu(actual, translated)
    #bleu=nltk.translate.bleu_score.sentence_bleu(actual[0],translated[0])
    #print (actual)
    #print (translated)
    #bs = bs/n_sentence
    print ('bleu= '+str(bleu))
