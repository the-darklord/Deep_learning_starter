...
def indexes_from_sentence(lang, sentence):
    return [lang.word2index[word] if word in lang.word2index else 1 for word in sentence.split(' ') ]
...
def evaluate_randomly(encoder, decoder):
    ...
    #lang1_lines = io.open('test/%s_%s/%s.txt' %(LANG1, LANG2, LANG1), encoding='utf8').read().strip().split('\n')
    #lang2_lines = io.open('test/%s_%s/%s.txt' %(LANG1, LANG2, LANG2), encoding='utf8').read().strip().split('\n')
    lang1_lines = io.open('generated/out/fr_ref', encoding='utf8').read().strip().split('\n')
    lang2_lines = io.open('generated/out/en_from_es', encoding='utf8').read().strip().split('\n')
    #lang1_lines= [l for l in lang1_lines if len(l.split(' ')) < MAX_LENGTH ]
    #lang1_lines=lang1_lines[:10000]
    pairs = [[normalize_string(s) for s in l] for l in zip(lang1_lines, lang2_lines)]
    pairs = [list(reversed(p)) for p in pairs]
    pairs = filter_pairs(pairs)
    #len(p[0].split(' ')) < MAX_LENGTH and len(p[1].split(' ')) < MAX_LENGTH #and   p[1].startswith(good_prefixes)

    actual=[]
    translated=[]
    bs=0.0
    n_sentence=min(len(pairs),5000)
    print (n_sentence)
    for i in range(n_sentence):
        ...
        pair = pairs[i]
        print(i)
        output_words, attentions = evaluate(' '.join(pair[0].split(' ')[:-1]), input_lang, output_lang, pairs, encoder, decoder)
        output_sentence = ' '.join(output_words)
        with open('generated/out/fr_from_en', 'a') as f:
          f.write(output_sentence)
          f.write('\n')
        with open('generated/out/fr_ref_2', 'a') as f:
          f.write(pair[1])
          f.write('\n')
        #with open('generated/out/es_src', 'a') as f:
        #  f.write(pair[0])
        #  f.write('\n')
        #print('>', pair[0])
        #print('=', pair[1].split(' '))
        ...
        #print('')
        actual.append([pair[1].split(' ')])
        translated.append(output_words[:-1])
    bleu=nltk.translate.bleu_score.corpus_bleu(actual,translated)
    print ('bleu= '+str(bleu))
'''
    print ('precisions= '+str(precisions))
    print ('bp= '+str(bp))
    print ('ratio= ' +str(ratio))
    print ('translation_length= '+str(translation_length))
